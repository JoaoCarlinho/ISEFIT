...
if(isset($_POST['p1'])  && isset($_POST['p2'])){
    ...
    $db = connect();
        ...
        if($count == 1){
            $query = $db->prepare("SELECT trainerFirstName FROM trainers WHERE email = :email AND newPasswordToken = :passToken LIMIT 1") or die("could not check member");
            $query ->bindParam(':email', $email, PDO::PARAM_STR);
